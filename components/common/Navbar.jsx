    ...
    <nav className="w-full h-16 border-b border-gray-300 bg-white shadow-sm fixed top-0 left-0 right-0 z-50">
      <div className="flex justify-between items-center h-16 px-4 sm:px-6 lg:px-8 w-full max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex gap-4 items-center">
          <Link href="/signin" className="text-gray-700 hover:text-blue-600 transition font-medium">
            Sign In
          </Link>
          <Link href="/signup" className="bg-blue-600 text-white rounded-full px-5 py-2 hover:bg-blue-700 transition font-medium">
            Sign Up
          </Link>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-200 shadow-lg flex flex-col p-4 gap-4 animate-in slide-in-from-top-2">
          ...
          <div className="flex flex-col gap-3 mt-2">
            <Link 
              href="/signin" 
              className="text-center text-gray-700 hover:text-blue-600 transition font-medium py-2 border border-gray-200 rounded-full"
              onClick={() => setIsMenuOpen(false)}
            >
              Sign In
            </Link>
            <Link 
              href="/signup" 
              className="text-center bg-blue-600 text-white rounded-full px-5 py-2 hover:bg-blue-700 transition font-medium"
              onClick={() => setIsMenuOpen(false)}
            >
              Sign Up
            </Link>
